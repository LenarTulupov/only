...
export default function Dates({ className, firstDate, lastDate }: IDates) {
  ...
  return (
    <div className={`${styles.dates} ${className || ''}`}>
      <Title className={styles.dates__title} color='blue' size='big'>{currentFirstDate}</Title>
      <Title className={styles.dates__title} color='pink' size='big'>{currentLastDate}</Title>
    </div>
  )
};
