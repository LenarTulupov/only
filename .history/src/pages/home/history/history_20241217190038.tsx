...
export default function HistorySection() {
  ...
  const firstDate = activeCategory?.events.map((event) => event.year[0]);
  const lastDate = activeCategory?.events.map((event) => event.year[-1]);
  ...
  return (
    <section className={styles.history}>
      <div className={styles.history__content}>
        ...
        />
        <Dates className={styles.dates} firstDate={firstDate}/>
      </div>
