...
export default function HistorySlider({ className, activeCategory }: IHistorySlider) {
  ...
  const [isBeginning, setIsBeginning] = useState(true); // Для отслеживания состояния Swiper
  const [isEnd, setIsEnd] = useState(false); // Для отслеживания состояния Swiper
  ...
  useEffect(() => {
    if (swiperRef.current && prevRef.current && nextRef.current) {
      ...
      // Добавление слушателей на изменение состояния Swiper
      ...
      <ContentContainer>
        ...
        {!isBeginning && (
          <div ref={prevRef} className={styles.prev}>
            <ArrowButton onClick={() => prevRef.current?.click()} slider />
          </div>
        )}
        {!isEnd && (
          <div ref={nextRef} className={styles.next}>
            <ArrowButton rotation="right" onClick={() => nextRef.current?.click()} slider />
          </div>
        )}
      </ContentContainer>
    </div>
