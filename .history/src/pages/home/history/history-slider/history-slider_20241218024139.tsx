...
export default function HistorySlider({ className, activeCategory }: IHistorySlider) {
  ...
  // Обновляем состояние навигации Swiper при смене категории
  useEffect(() => {
    if (swiperRef.current) {
      swiperRef.current.slideTo(0); // Перемещаем слайдер на первый слайд
  ...
  return (
    ...
    >
      <ContentContainer>
        <Swiper
          key={activeCategory?.id} // Переинициализируем Swiper
          spaceBetween={80}
          slidesPerView={3.5}
          modules={[Navigation]}
          onSwiper={(swiper) => {
