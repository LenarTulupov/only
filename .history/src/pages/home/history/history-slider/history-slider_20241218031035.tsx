...
export default function HistorySlider({ className, activeCategory }: IHistorySlider) {
  ...
  useEffect(() => {
    if (prevCategory?.id !== activeCategory?.id) {
      const ctx = gsap.context(() => {
        ...
        // Анимация исчезновения старой категории
        timeline
          .to(containerRef.current, {
            ...
            ease: "power2.in",
            onComplete: () => {
              // После завершения анимации исчезновения обновляем категорию и запускаем анимацию появления
  ...
  useEffect(() => {
    if (swiperRef.current) {
      swiperRef.current.slideTo(0, 0); // Reset to the first slide
    }
  }, [activeCategory]);
      ...
      <ContentContainer>
        ...
        {!isBeginning && (
          <div ref={prevRef} className={styles.prev}>
            <ArrowButton
              onClick={() => swiperRef.current?.slidePrev()} // Use Swiper method directly
              slider
        ...
        {!isEnd && (
          <div ref={nextRef} className={styles.next}>
            <ArrowButton
              rotation="right"
              onClick={() => swiperRef.current?.slideNext()} // Use Swiper method directly
              slider
              disabled={isEnd}
            />
          </div>
